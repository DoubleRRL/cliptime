...
async def create_task(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if llm_model is not None:
        from ...ai import VALID_MODEL_PATTERN

        llm_model = str(llm_model).strip() or None
        if llm_model and not VALID_MODEL_PATTERN.match(llm_model):
            raise HTTPException(
                status_code=400,
                detail="Invalid llm_model format. Expected 'provider:model-name'.",
            )
...
async def get_task_progress_sse(task_id: str, request: Request):
    ...
    async def event_generator():
        ...
        # Subscribe to progress updates via the shared pooled client; the
        # pub/sub connection itself is closed by subscribe_to_progress.
...
async def delete_clip(
    ...
    try:
        ...
        from ...file_cleanup import safe_unlink

        safe_unlink(clip.get("file_path", ""))
...
async def re_render_clip(
    ...
    try:
        ...
        end_delta = float(payload.get("end_delta_seconds", 0))
        font_family = _normalize_font_family(
